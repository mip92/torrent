...
import {AddGanreDto} from "../ganres/dto/add-ganre-dto";
...
export class TorrentController {
    ...
    create(@UploadedFiles() files, @Body() dto: CreateTorrentDto) {
        ...
        if(!picture0 || !picture1 || !picture2 || !picture3)  {
            throw new HttpException({message: ["Количество скриншотов меньше чем 4"]}, HttpStatus.BAD_REQUEST)
        }
        if(!mainPicture) {throw new HttpException({message: ["Нет главной картинки"]}, HttpStatus.BAD_REQUEST)}
        if(!torrent)  {throw new HttpException({message: ["Нет торрент файла"]}, HttpStatus.BAD_REQUEST)}
